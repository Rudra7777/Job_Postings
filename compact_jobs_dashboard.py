...
def load_data():
    try:
        # Load Excel file
        url = "https://www.dropbox.com/scl/fi/synbz0e1i3hv9pnbfts31/job_data_fully_cleaned.parquet?rlkey=duxgi5e5glfnu58lt9gjuow4h&st=3i76resi&dl=1"
        ...
        # Create experienceCategory column
        df['experienceCategory'] = df['avgExperience'].apply(categorize_experience)
        
        return df
    
    except FileNotFoundError:
        st.error("❌ File not found! Make sure 'job_data_fully_cleaned.xlsx' is in the same folder.")
        return pd.DataFrame()
